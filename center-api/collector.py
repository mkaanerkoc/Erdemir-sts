...
def FindLastEntry(channelID,Interval):
    ...
    if channelID > 10:
        return 0x00
    cursor = local_db['posts.d'+str(channelID)].find({}).sort([("serverDt",pymongo.DESCENDING)]).limit(1)
...
def GetOldDatas(channelID,itemCount):
    ...
    cursor = local_db['posts.d'+str(channelID)].find({}).sort([("serverDt",pymongo.DESCENDING)]).limit(itemCount)
    aboveRow=0
    belowRow=0
    aboveRow = cursor[0]
    dumps(aboveRow)
    ...
    for belowRow in cursor:
        dumps(belowRow)
        belowRow['serverDt'] = belowRow['serverDt'].replace(second=channelID*5, microsecond=0)
        aboveRow['serverDt'] = aboveRow['serverDt'].replace(second=channelID*5, microsecond=0)
        ...
        if timeDiff>1 and timeDiff < 180:
            ...
            for i in range(1,timeDiff):
                ...
                local_db['posts.d'+str(channelID)].insert_one({'serverDt':aboveRow['serverDt'] - timedelta(minutes=i),'nodeDt':(aboveRow['serverDt'] - timedelta(minutes=i)).strftime('%H:%M:%S-%d/%m/%Y'),'devID':channelID,'temp1':lastTemp1,'temp2':lastTemp2,'tempC':lastTempC,'humdC':lastHumdC
                ,'rssi':lastRSSI+random.randint(-10,10),'mCrc':random.randint(5000,40000)})
...
def InsertNewItemsToLocal(response,channelID):
    ...
    for l in response:
        ...
        local_client = MongoClient("localhost",27017)
        local_db = local_client['test-database']
        collection = local_db['posts.d'+str(channelID)]
